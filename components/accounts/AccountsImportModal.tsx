...
export default function AccountsImportModal({ onClose }: Props) {
  ...
  function handleReconcileConfirm() {
    ...
    // Zustand set() is synchronous — mutations are immediately visible via getState()
    reconcileResult.incoming.forEach((def) => addFieldDef(def));
    fieldsToRemove.forEach((id) => removeFieldDef(id));

    // Read the updated fieldDefs with their real store-assigned IDs
